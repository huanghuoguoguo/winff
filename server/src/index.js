...
const { createStreamRouter } = require('./stream');
...
let appConfig = {
    port: 3000,
    sharedDirs: {
        "默认共享": path.join(require('os').homedir(), 'Desktop')
    }
};
...
// 找一个默认的主共享目录，作为上传存放地
const defaultShare = Object.values(SHARED_DIRS)[0] || path.join(require('os').homedir(), 'Desktop');
